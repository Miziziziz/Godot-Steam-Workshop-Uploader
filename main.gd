...
func _ready() -> void:
	...
	SteamService.log_message.connect(log_in_console)
	SteamService.tags_set.connect(on_tags_set)
	SteamService.initialize()
	
	Steam.item_created.connect(on_workshop_mod_created)
	Steam.item_updated.connect(on_workshop_mod_updated)
...
func on_workshop_mod_updated(result: int, needs_to_accept_agreement: bool, file_id: int) -> void:
	if result == 1: log_in_console("Item successfully uploaded.")
	else: log_in_console("Item upload has failed: %s" % result)
